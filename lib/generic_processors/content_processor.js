...
function process(contentWithLocals, contentType, renderTemplate, config, db) {
  return db.get('locals')[contentType].map((current, i) => {
    const page = contentWithLocals[i];
    // current are the locals for the current page
    // locals contains all pages/posts other content types etc.
    const locals = Object.assign({}, db.get('locals'), { current });
