...
function init(templateProcessors, templates, defaultOpts, helpers) {
  function processTemplate(templateName, locals, opts) {
    if (templates.has(templateName)) {
      ...
      for (const [processorName, processor] of templateProcessors.entries()) {
        ...
        if (extIndex !== -1) {
          ...
          return processor.process(templateObject.template, mergedOptions, locals, helpers);
        }
      }
      throw new Error(`No template processor found for extension:
${templateObject.ext}. Given extension ${templateObject.ext}`);
    } else {
