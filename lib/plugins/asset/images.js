...
function init({
  ...
  function read(basePath, { readers }) {
    const imagePaths = readers.dirRecursive(`${basePath}`);
    return imagePaths.map((imagePath) => ({
      id: path.basename(imagePath),
      name: path.basename(imagePath),
      path: imagePath.replace(basePath, '').replace(path.basename(imagePath), ''),
  ...
  function preprocess(images) {
    return images.map((image) => {
      ...
      next.writePath = path.join('images', image.path, image.name);
      return next;
    });
  }
