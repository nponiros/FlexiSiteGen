...
function init({
  ...
  function normalizePostcssPluginPaths(basePath, plugins) {
    return plugins.map((plugin) => {
      ...
      return Object.assign({}, plugin, { name: path.resolve(basePath, 'node_modules', plugin.name) });
    });
  }
  ...
  function preparePostcssOptions(opts = {}, basePath) {
    ...
    if (opts.parser) {
      postcssOptions.parser = require(normalizePostcssPluginPaths(basePath, opts.parser));
    }

    if (opts.syntax) {
      postcssOptions.syntax = require(normalizePostcssPluginPaths(basePath, opts.syntax));
    }
  ...
  function postprocess(styles, helperFunctions, db) {
    ...
    const postcssNormalizedPlugins = normalizePostcssPluginPaths(db, postcssPlugins);
