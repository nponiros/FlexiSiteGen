        ...
        return [...results, ...postprocessedBundles]
          .map((style) => {
            ...
            if (config.cacheBust) {
              ...
              // In some cases a postcss plugin can empty a .css file.
              // Use a comment to make sure that if style.css is empty, fingerprint does not try to read the
              // file from disk.
              const contents = style.css || '/* Empty */';
              const filename = helperFunctions.fingerprint(style.name, contents, fingerprintOpts).file;
