...
function init({
  ...
  function locals(pages, { createStyles, createScripts }, db) {
    return pages.map((page) => {
      const styles = createStyles(config.styles, page.meta, db.get('locals').styles.object);
      const scripts = createScripts(config.scripts, page.meta, db.get('locals').scripts.object);
