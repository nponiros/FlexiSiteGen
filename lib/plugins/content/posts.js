...
function init({
  ...
  function createLocals(posts, fns, deps) {
    return posts.map((post) => {
      const styles = fns.createStyles(config.styles, post.meta, deps.styles.object);
      const scripts = fns.createScripts(config.scripts, post.meta, deps.scripts.object);
