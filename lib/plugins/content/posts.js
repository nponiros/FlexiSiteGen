...
function init({
  ...
  function locals(posts, { createStyles, createScripts }, db) {
    return posts.map((post) => {
      const styles = createStyles(config.styles, post.meta, db.get('locals').styles.object);
      const scripts = createScripts(config.scripts, post.meta, db.get('locals').scripts.object);
