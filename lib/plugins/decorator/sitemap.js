...
function init({
  ...
  function locals(sitemap, fns, db) {
    const sitemapLocals = [];
    const locals = db.get('locals');

    return config.createMapFor
      .map((type) => locals[type].map((content) => content.meta.url))
  ...
  function process(sitemapWithLocals, helperFns, db) {
    ...
    next.contentsToWrite = createSitemap(domain, db.get('locals').sitemap);
    return next;
  }
